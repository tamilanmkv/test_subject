...
const userNameorEmail = (input) => {
  if (input.attrs.type == 'text'){
    if (input.attrs.id){
      console.log(input.attrs.id)         //login-username
...
const userSubmitButton = (input,submitButton) => {
  if (submitButton){
    console.log(submitButton.attrs)
  }
...
const LoginPage =  (loginHtml) => {
    ...
    let submitButton = soup.findAll('button').map((button) => (button.attrs.type == 'submit')?console.log(button.attrs.id):console.log("badass"));
