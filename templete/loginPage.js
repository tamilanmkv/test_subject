...
const loginHtml2 = `
<html id="app" lang="en" dir="ltr"><head>
<meta charset="utf-8">
...
const userNameorEmail = (input) => {
  if (input.attrs.type == 'text'){
    inputIdOrClass(input);
  }else if (input.attrs.type == 'email'){
    inputIdOrClass(input);
  }else if (input.attrs.type == 'username'){
    inputIdOrClass(input);
  }
...
const userSubmitButton = (input,submitButton) => {
  if (input.attrs.type == 'submit'){
    ...
    submitButton.map((button) => {
      if (button.attrs){
        if ((button.attrs.id || button.attrs.class).includes('login')){
          buttonIdOrClass(button);
        }
        else if((button.attrs.id || button.attrs.class).includes('sign')){
          if((button.attrs.id || button.attrs.class).includes('up')){
            console.log("button is sign up");
          }else if((button.attrs.id || button.attrs.class).includes('in')){
              buttonIdOrClass(button);
           }else{
            console.log("button is sign use enter keyword");
             }
          }
        }else{
          console.log("no attribute found");
...
const LoginPage =  (loginHtml) => {
    const soup = new beautifulSoup(loginHtml);
    let formTags = soup.findAll('form')
    let loginInput = soup.findAll('input')
    let submitButton = soup.findAll('button');
    // let inputTags = formTags.map(input => console.log(input.findAll('input')))
    if (formTags.length > 0 ){
      for (let i = 0; i < formTags.length; i++){
        let inputTags = formTags[i].findAll('input');
        let buttonTags = formTags[i].findAll('button');
        if (inputTags.length > 0){
          inputTags.map((input) => {
            if (input.attrs){
              if (input.attrs.type == 'text' || input.attrs.type == 'email' || input.attrs.type == 'username'){
                userNameorEmail(input);
              }
              else if (input.attrs.type == 'password'){
                userPassword(input);
              }
              else if (input.attrs.type == 'submit' ||  buttonTags.length > 0){
                userSubmitButton(input,submitButton);
              }
            }
          });
        }
      }
      // console.log(formTags.map(input =>{
      //   input.findAll('input').map(input => {
      //     userNameorEmail(input);
      //     userPassword(input);
      //     userSubmitButton(input,submitButton);
      ...
      //   checked: ''
      // }
      for (let input of loginInput){        //text        password        checkbox
        console.log(input.attrs.type);
        userNameorEmail(input);
        userPassword(input);
        userSubmitButton(input,submitButton)
        }
